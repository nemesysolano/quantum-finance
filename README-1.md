...
In order to assess how much improvement quantum mechanics can add to existing neural network models, we are going to draft a basic **Deep Neural Network (DNN)** model.

### Prediction Target ###

The model is a binary classifier that aims to predict the sign of the close price change at time ${t}$. The target variable is defined as:

$\mathbf{target}(t) = 1$ when $p(t) - p(t-1) > 0$, or

$\mathbf{target}(t) = -1$ when $p(t) - p(t-1) < 0$, finally

$\mathbf{target}(t) = 0$ when $p(t) - p(t-1) = 0$

### Input Features

The model uses a fixed **lookback window of $k$ bars** (from $t-1$ to $t-k$). 
The input features are the **Price-Volume Strength Oscillator** calculated for close ($Y_c$) and difference between **directional probabilities** ($P_d$).

The input for a single prediction at time $t$ is the concatenation of these **five** k-bar sequences:

| Feature | $t-1$ | $t-2$ | ... | $t-k$ |
| :--- | :--- | :--- | :--- | :--- |
| ${Y_c}$ | $Y_c(t-1)$ | $Y_c(t-2)$ | ... | $Y_c(t-k)$ |
| ${P_d}$ | $P_↑(t-1) - P_↓(t-1)$ | $P_↑(t-2) - P_↓(t-2)$ | ... | $P_↑(t-k) - P_↓(t-k)$ |

---
Our strategy is to first establish a robust classical foundation by tuning the DNN model for optimal accuracy. Subsequently, this model will serve as the starting point for enhancement through the introduction of quantum mechanics, allowing us to explore the performance gains of quantum-inspired algorithms.